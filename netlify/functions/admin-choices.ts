...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // GET: list choices by question_id
    if (method === "GET") {
      const question_id = String(event.queryStringParameters?.question_id || "").trim();
      if (!question_id) return json(400, { error: "Missing question_id" });
